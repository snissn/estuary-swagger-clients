...
(defn content-add-car-post-with-http-info
  ...
  ([body {:keys [filename commp size ]}]
   (check-required-params body)
   (call-api "/content/add-car" :post
             {:path-params   {}
              :header-params {}
              :query-params  {"filename" filename "commp" commp "size" size }
              :form-params   {}
              :body-param    body
              :content-types []
...
(defn content-add-ipfs-post
  ...
  This endpoint is used to add an IPFS object to the network. The object can be a file or a directory."
  [body ]
  (:data (content-add-ipfs-post-with-http-info body)))
...
(defn content-create-post
  ...
  This endpoint adds a new content"
  [body ]
  (:data (content-create-post-with-http-info body)))
...
(defn content-stats-get
  ...
  This endpoint is used to get content statistics. Every content stored in the network (estuary) is tracked by a unique ID which can be used to get information about the content. This endpoint will allow the consumer to get the collected stats of a conten"
  [limit ]
  (:data (content-stats-get-with-http-info limit)))
